...
const cartCountEl = () => document.getElementById('cart-count'); // CAMBIO: helper para contador
...
function renderCart(cartItems, cartTotal) {
  ...
  if (cartState.size === 0) {
    ...
    cartCountEl()?.textContent = '0'; // CAMBIO: sincroniza contador al vaciar
    return;
  }
  ...
  cartCountEl()?.textContent = `${Array.from(cartState.values()).reduce((acc, item) => acc + item.quantity, 0)}`; // CAMBIO: total de unidades
}

function addToCart(productId, cartItems, cartTotal, cart, backdrop) {
...
function openCart(cart, backdrop) {
  cart?.classList.add('is-open'); // CAMBIO: mostramos panel del carrito
  backdrop?.classList.add('is-visible'); // CAMBIO: activamos fondo clicable
}

function closeCart(cart, backdrop) {
  cart?.classList.remove('is-open'); // CAMBIO: ocultamos panel del carrito
  backdrop?.classList.remove('is-visible'); // CAMBIO: ocultamos fondo
}

function handleAddClicks(event, cartItems, cartTotal, cart, backdrop) {
  const button = event.target.closest('[data-add]');
  const addId = button?.getAttribute('data-add');
  if (addId) {
    addToCart(addId, cartItems, cartTotal, cart, backdrop);
  }
}
...
function initWyvernStore() {
  ...
  document.addEventListener('click', (event) => handleAddClicks(event, cartItems, cartTotal, cart, cartBackdrop));
  ...
  cartToggle?.addEventListener('click', () => {
    const shouldOpen = !cart?.classList.contains('is-open');
    if (shouldOpen) {
      openCart(cart, cartBackdrop); // CAMBIO: abrir desde el botón de navbar
    } else {
      closeCart(cart, cartBackdrop); // CAMBIO: cerrar desde el botón de navbar
    }
  });

  closeCartBtn?.addEventListener('click', () => closeCart(cart, cartBackdrop)); // CAMBIO
  cartBackdrop?.addEventListener('click', () => closeCart(cart, cartBackdrop)); // CAMBIO
}
